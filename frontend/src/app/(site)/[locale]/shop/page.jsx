...
export default async function ShopPage({ params, searchParams }) {
  ...
  return (
    <>
      ...
      <section className="grid-container py-12"><div><CatalogToolbar locale={locale} activeAudience={null} /><ProductGrid locale={locale} filters={{ type: query.type, sale: query.sale, sort: query.sort, material: query.material, shape: query.shape, feature: query.feature, frameType: query.frameType, frameColor: query.frameColor, lensColor: query.lensColor, size: query.size, brand: query.brand, priceMin: query.priceMin, priceMax: query.priceMax, search: query.q }} /></div></section>
    </>
  );
}
