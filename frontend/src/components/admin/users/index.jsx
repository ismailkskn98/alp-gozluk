...
export default function AdminUserManagement() {
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader eyebrow="Erişim yönetimi" title="Yönetici kullanıcılar" description="Yeni panel hesaplarını yalnızca süper yönetici oluşturabilir. Public kayıt akışı admin yetkisi vermez." actions={<Sheet open={createOpen} onOpenChange={setCreateOpen}><SheetTrigger asChild><AdminButton><Plus className="size-4" />Kullanıcı oluştur</AdminButton></SheetTrigger><SheetContent className="w-[min(96vw,38rem)] overflow-y-auto border-border bg-card text-card-foreground sm:w-[38rem]"><SheetHeader className="border-b border-border px-5 py-5 sm:px-6"><SheetTitle>Yönetici kullanıcı oluştur</SheetTitle><SheetDescription>Süper yönetici rolü buradan atanamaz. Yeni hesapta güçlü bir geçici şifre kullanın.</SheetDescription></SheetHeader><div className="p-5 sm:p-6"><CreateUserForm compact roles={roles} onCreated={async (user, error) => { setMessage(error || (user ? 'Kullanıcı oluşturuldu.' : '')); if (user) { setCreateOpen(false); await load(); } }} /></div></SheetContent></Sheet>} />
