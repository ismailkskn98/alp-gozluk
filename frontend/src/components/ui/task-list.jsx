...
function TaskCheck({
  ...
  return (
    <motion.svg
      viewBox="0 0 24 24"
      aria-hidden
      className={cn("shrink-0 text-neutral-300 dark:text-neutral-600", SIZES[size].check)}
      initial={false}
...
export function TaskItem({
  ...
  const [stage, setStage] = useState(done ? STAGE.settled : STAGE.idle);
  ...
  return (
    <motion.button
      ...
      {...props}>
      <TaskCheck filled={FILLED.includes(stage)} size={size} onDrawn={onDrawn} />
      <TaskLabel
        label={label}
...
export function TaskList({
  ...
  return (
    <ul
      data-slot="task-list"
      className={cn("flex w-fit max-w-full flex-col items-start", SIZES[size].list, className)}
      {...props}>
      {[...open, ...finished].map((task) => (
        <motion.li key={task.id} layout transition={timing(REORDER)} className="max-w-full">
          <TaskItem
            label={task.label}
            checked={!!task.done}
            size={size}
