...
function badgeMetrics(size, dot) {
...
function BellIcon({ side, swing, clapper }) {
  return (
    <motion.svg
      viewBox="0 0 18 18"
      fill="currentColor"
...
function DigitColumn({ value, reduced }) {
    ...
    >
      <motion.span className="absolute inset-0" style={{ y }}>
        {Array.from({ length: WINDOW * 2 + 1 }, (_, i) => {
          ...
          return (
            <span key={tile} className="absolute inset-x-0 flex justify-center" style={{ top: `${tile * 100}%` }}>
              {digitOf(tile)}
            </span>
          );
        })}
...
function CountBadge({ total, max, size, color, dot, reduced }) {
    ...
    <AnimatePresence initial={false}>
      {total > 0 && (
        <motion.span
          ...
          layout={!reduced}
          aria-hidden
          className={cn("pointer-events-none absolute z-10 grid place-items-center rounded-full", COLORS[color])}
          style={{
            top: inset,
            right: inset,
            height: side,
            ...
            >
              {clamped
                ...
                : Array.from({ length: places }, (_, i) => {
                    ...
                    return <DigitColumn key={place} value={Math.floor(total / 10 ** place)} reduced={reduced} />;
                  })}
            </span>
          )}
        </motion.span>
...
export function NotificationBell({ count = 0, max = 99, variant = "count", size = 48, color = "red", asChild = false, className, style, children, ...props }) {
  const reduced = useReducedMotion() ?? false;
  const total = Number.isFinite(count) ? Math.max(0, Math.floor(count)) : 0;
  // use the total so a weird count cannot ring the bell
  const { swing, clapper } = useBellRing(total, reduced);

  const badge = <CountBadge total={total} max={max} size={size} color={color} dot={variant === "dot"} reduced={reduced} />;
  ...
  if (asChild) {
    return (
      <Slot data-slot="notification-bell" className={cn("relative", className)} style={style} {...props}>
