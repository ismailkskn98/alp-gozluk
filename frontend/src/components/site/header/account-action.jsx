...
export default function AccountAction({ authenticated, locale, label }) {
  ...
  if (authenticated) {
    return (
      <Link href="/account" aria-label={label} className="grid size-11 place-items-center hover:bg-[#f4f5f6]">
  ...
  return (
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetTrigger asChild>
        <button type="button" aria-label={label} className="grid size-11 place-items-center hover:bg-[#f4f5f6]">
          <UserRound className="size-4" strokeWidth={1.5} />
        </button>
      </SheetTrigger>
      <SheetContent
        side="right"
        closeLabel={t('close')}
        className="w-full gap-0 border-l border-border bg-white shadow-none sm:w-[min(32rem,100vw)]"
      >
        <div className="flex min-h-0 flex-1 flex-col overflow-y-auto">
          <SheetHeader className="border-b border-border px-6 pb-6 pt-16 sm:px-9 sm:pb-8 sm:pt-20">
            ...
            <SheetTitle className="max-w-sm text-[clamp(2.25rem,7vw,3.75rem)] font-light leading-[0.94] tracking-[-0.045em]">
              {t('sheetLoginTitle')}
            </SheetTitle>
            <SheetDescription className="max-w-md pt-2 text-sm leading-6">
              {t('sheetLoginDescription')}
            </SheetDescription>
          </SheetHeader>
