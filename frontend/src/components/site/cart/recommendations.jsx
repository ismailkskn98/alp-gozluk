  ...
  return (
    <section className="mt-[clamp(2.5rem,5vw,5rem)] border-t border-border pt-7" aria-labelledby="cart-recommendations-title">
      <div className="flex items-end justify-between gap-6">
        <h2 id="cart-recommendations-title" className="text-2xl leading-none sm:text-3xl">
          {labels.recommendations}
        </h2>
        <Link href="/shop" className="hidden border-b border-foreground pb-1 text-sm sm:block">
          {labels.viewAll}
        </Link>
      </div>
          ...
          <article key={`${product.slug}-${product.image}`} className="min-w-0">
            <Link href={`/product/${product.slug}`} className="group block">
              <div className="relative aspect-[4/3] overflow-hidden ">
                <Image src={product.image} alt={product.name} fill quality={90} className="object-contain p-[4%] transition-transform duration-500 group-hover:scale-[1.025]" />
              </div>
