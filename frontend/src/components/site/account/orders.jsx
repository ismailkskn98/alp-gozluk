...
import { ArrowLeft, ArrowRight, Box, PackageOpen, RotateCcw, Truck } from 'lucide-react';
...
function ProductThumbs({ products = [] }) {
  if (!products.length) return <span className="grid size-20 place-items-center bg-white text-[#87918d]"><Box className="size-5" strokeWidth={1.35} /></span>;
  return (
  ...
  return (
    <div className="grid grid-cols-[5rem_minmax(0,1fr)] gap-4 border-b border-[#e0e4e1] py-4 last:border-0 sm:grid-cols-[5rem_minmax(0,1fr)_auto] sm:items-center">
      <div className="relative aspect-[4/5] bg-white">
        {product.imageUrl ? <Image src={product.imageUrl} alt={product.name} fill sizes="80px" className="object-contain" /> : null}
      </div>
      ...
      <p className="col-start-2 text-sm font-medium text-[#172536] tabular-nums sm:col-auto">
        {formatMoney(product.priceAmount, product.currency || 'TRY', locale)}
      </p>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="mt-5 flex flex-col gap-5 border-y border-[#d8ddd7] py-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        <AnimatedBadge status={state.badge} size="sm">{state.label}</AnimatedBadge>
      </div>
      <div className="grid gap-[clamp(2rem,5vw,4.5rem)] py-6 lg:grid-cols-[minmax(0,1fr)_17rem]">
        <div>
          ...
          <div className="mt-2">{(order.items || []).map((product) => <ProductLine key={product.id || product.slug} product={product} locale={locale} />)}</div>
        </div>
        <dl className="space-y-4 border-l border-[#d8ddd7] pl-5 text-sm">
          <div><dt className="text-xs text-[#68736f]">Teslimat</dt><dd className="mt-1 text-[#172536]">{order.address || 'Kayıtlı teslimat adresi'}</dd></div>
          {order.shipment ? <div><dt className="text-xs text-[#68736f]">Kargo</dt><dd className="mt-1 text-[#172536]">{order.shipment.company}<br /><span className="tabular-nums">{order.shipment.trackingNumber}</span></dd></div> : null}
          <div className="flex items-end justify-between border-t border-[#d8ddd7] pt-4"><dt className="text-[#68736f]">Toplam</dt><dd className="text-base font-medium text-[#172536] tabular-nums">{formatMoney(order.totalAmount, order.currency, locale)}</dd></div>
        </dl>
      </div>
    </motion.section>
  );
        ...
        return (
          <button key={order.orderNumber} type="button" onClick={() => onSelect(order)} className="group grid w-full gap-5 py-5 text-left md:grid-cols-[auto_minmax(0,1fr)_auto] md:items-center">
            ...
            <span className="min-w-0">
              ...
              <span className="mt-2 block text-sm text-[#68736f]">{formatDate(order.placedAt, locale)} · {order.itemCount} ürün</span>
            </span>
        ...
        return (
          <article key={request.returnNumber} className="grid gap-5 py-5 md:grid-cols-[auto_minmax(0,1fr)_auto] md:items-center">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-[#68736f]">Sipariş #{request.orderNumber} · {formatDate(request.requestedAt, locale)}</p>
...
function EmptyState({ icon: Icon, title, description }) {
  return (
    <div className="border-y border-[#d8ddd7] py-14 text-center">
      ...
      <Link href="/shop" className="mt-5 inline-flex items-center gap-2 text-sm font-medium text-[#172536]">Gözlükleri keşfet <ArrowRight className="size-3.5" /></Link>
    </div>
  );
}
...
export default function Orders({ orders, returns, locale }) {
  ...
  const reduceMotion = useReducedMotion();
  const tabs = useMemo(() => [
    { label: `Siparişler (${orders.length})`, icon: <Truck /> },
    { label: `İadeler (${returns.length})`, icon: <RotateCcw /> },
  ], [orders.length, returns.length]);

  return (
    <section>
      <AccountSectionHeader
        kicker="Alışveriş geçmişin"
        title="Siparişlerim"
        description="Kargonu takip et, sipariş ayrıntılarını incele ve iade süreçlerini tek yerden gör."
      />
      <div className="mt-6 overflow-x-auto pb-1">
        <GooeyNav items={tabs} value={tab} onChange={(value) => { setTab(value); setSelectedOrder(null); }} size="sm" activeColor="#172536" separation={10} radius={8} aria-label="Sipariş ve iade kayıtları" />
      </div>
      <div className="mt-6">
        <AnimatePresence mode="wait" initial={false}>
          <motion.div key={`${tab}-${selectedOrder?.orderNumber || 'list'}`} initial={reduceMotion ? false : { opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={reduceMotion ? undefined : { opacity: 0, y: -4 }} transition={{ duration: 0.18 }}>
            {selectedOrder
